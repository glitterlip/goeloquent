...
func TestSelectPivot(t *testing.T) {
	...
	RunWithDB(before, after, func(conn goeloquent.Connection) {
		...
		_, err = conn.Model(&goeloquent.EloquentModel{}).Table("role_user").Insert([]map[string]interface{}{
			{"role_id": 1, "user_id": 2, "valid": 1},
			{"role_id": 1, "user_id": 1, "valid": 0},
			{"role_id": 2, "user_id": 3, "valid": 1},
			{"role_id": 2, "user_id": 4, "valid": 0},
			{"role_id": 3, "user_id": 5, "valid": 0},
			{"role_id": 4, "user_id": 5, "valid": 1},
			{"role_id": 5, "user_id": 7, "valid": 0},
		})
		assert.Nil(t, err)
		var sts []*goeloquent.Statement
		goeloquent.DB.Listen(goeloquent.EventQueryExecuted, func(name goeloquent.EventName, i ...interface{}) bool {
		...
		var roles1 []BelongsToManyRoles
		sts = []*goeloquent.Statement{}
		_, err = conn.Model(&BelongsToManyRoles{}).With("Users").WithPivot("valid").WherePivot("valid", 1).Get(&roles1)
		assert.Nil(t, err)
		assert.Equal(t, 2, len(sts))
		assert.Equal(t, "select `users`.*, `role_user`.`role_id` as `goelo_pivot_role_id`, `role_user`.`user_id` as `goelo_pivot_user_id`, `role_user`.`valid` as `goelo_pivot_valid` from `users` inner join `role_user` on `role_user`.`user_id` = `users`.`id` where `role_user`.`role_id` in (?, ?, ?, ?, ?) and `valid` = ?", sts[1].RawSql)
		assert.Equal(t, []interface{}{int64(1), int64(2), int64(3), int64(4), int64(5), 1}, sts[1].GetBindings())
		assert.Equal(t, 5, len(roles1))
		for _, role := range roles1 {
			if role.Id == 1 || role.Id == 2 || role.Id == 4 {
				assert.NotEmpty(t, role.Users)
				for _, user := range role.Users {
					assert.Equal(t, int32(role.Id), user.Pivot["role_id"])
					assert.Equal(t, int32(1), user.Pivot["valid"])
				}
			}
		}
